...
#            иначе автоматический апгрейд «зависает» на вводе. Логин/пароль
#            по-прежнему печатаются в вывод (и попадают в лог обновления).
# ============================================================================
...
if [ "$PASSWORD_SHOWN" = true ]; then
    # 🆕 v2.0.3: при автоматической докатке из upgrade.sh (SAMBA_UPGRADE=1) паузы НЕТ —
    # иначе апгрейд «зависает» на вводе. Логин/пароль уже напечатаны выше и попали
    # в лог обновления; сменить пароль можно в любой момент: sudo smbpasswd smbuser
    if [ "${SAMBA_UPGRADE:-0}" = "1" ]; then
        echo -e "${YELLOW}ℹ️  Докатка из upgrade.sh: паузы нет. Пароль — в выводе выше (сменить: sudo smbpasswd ${SMB_USER})${NC}"
    elif [ -t 0 ]; then
        # Пауза, чтобы пользователь успел записать данные (только интерактивно)
