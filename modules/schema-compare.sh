#!/bin/bash
# ============================================================================
# modules/schema-compare.sh v1.1.0 — удалённая диагностика расхождений схемы
# ============================================================================
...
#   - v1.1.0: количество видов «эталон N, у клиента M» — СПРАВОЧНО, не ошибка
#     (виды и раскладка UI — «эталон-онли»: клиент настраивает их сам,
#     см. upgrades/README.md → «Что мигрируем дельтами, а что живёт только в эталоне»).
# ============================================================================
set -euo pipefail
...
out "═══════════════════════════════════════════════════════════"
out "🔍 schema-compare.sh v1.1.0 — расхождения с эталоном"
out "   База:   $NOCO_DB"
out "   Эталон: $TEMPLATE"
out "═══════════════════════════════════════════════════════════"
...
# --- 5. ВИДЫ (эталон-онли — справочно, НЕ ошибка) ---
# Правило миграции: состав (таблицы/колонки/связи/опции) догоняется дельтами, а
# виды и раскладка UI — нет: клиент настраивает их под себя (upgrades/README.md).
# Поэтому расхождение по видам здесь только информационное — сверять и «догонять» его
# не нужно. Зачем показываем: чтобы вопрос «а почему у клиента нет вида X?» закрывался
# сразу ответом, а не превращался в «забыли дельту».
out ""
out "👁  ВИДЫ (эталон-онли — расхождение НЕ ошибка, дельта не нужна)"
VIEW_BASE_FILTER="m.title NOT LIKE 'nc\\_%' ESCAPE '\\' AND m.title!='workspace' AND m.table_name!='' AND m.table_name IS NOT NULL AND m.mm!=1"
ref_views=$(q "$TEMPLATE" "SELECT COUNT(*) FROM nc_views_v2 v JOIN nc_models_v2 m ON m.id=v.fk_model_id WHERE $VIEW_BASE_FILTER;")
live_views=$(q "$NOCO_DB" "SELECT COUNT(*) FROM nc_views_v2 v JOIN nc_models_v2 m ON m.id=v.fk_model_id WHERE $VIEW_BASE_FILTER;")
out "  ℹ️ видов: эталон $ref_views, у клиента $live_views"
if [ "$ref_views" != "$live_views" ]; then
    out "     (разница не требует действий: клиент создаёт/удаляет виды сам)"
fi
