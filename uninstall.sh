...
# v4.47.2: образы удаляются ПО ИМЕНИ РЕПОЗИТОРИЯ (любые теги) — раньше список был
# захардкожен под nocodb/nocodb:2026.06.2 (после апгрейдов NocoDB образ не удалялся),
# а bot/webhook с тегом «:latest» вообще не матчились паттерном без тега.
for repo in printed4u-crm-bot printed4u-crm-webhook nocodb/nocodb; do
    for img in $(docker images --format '{{.Repository}}:{{.Tag}}' 2>/dev/null | grep "^${repo}:" || true); do
        docker rmi -f "$img" 2>/dev/null || true
        echo -e "${GREEN}   ✅ Образ $img удалён${NC}"
    done
done
