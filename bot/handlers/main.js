    ...
    try {
        ...
        if (data.startsWith('pitem_')) {
            ...
            if (kind === 'new') { // pitem_new_{pid}
                ...
                const types = [['🔧 Работа', 'Работа'], ['📦 Товар', 'Товар'], ['🏭 Изделие', 'Изделие'], ['🧱 Мат. заказчика', 'Мат. заказчика']];
                ...
                bot.sendMessage(chatId,
                    '🏷 *Что это за позиция?*\n\n' +
                    '🔧 Работа — услуга/печать/монтаж → в счёт и АКТ\n' +
                    '📦 Товар — готовое изделие/материал → в счёт и НАКЛАДНУЮ\n' +
                    '🏭 Изделие — изготовил под заказ → в счёт и НАКЛАДНУЮ (в акт не попадает)\n' +
                    '🧱 Мат. заказчика — материал клиента, БЕЗ цены, в счёт не входит\n\n' +
                    '_Имя везде из «Название». Если в накладной нужно другое имя (обычно для «Изделия») — заполни «Название (ТН)» в NocoDB._',
