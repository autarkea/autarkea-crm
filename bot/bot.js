...
async function sendProjectItemDetails(chatId, messageId, itemId, projectId) {
    try {
        ...
        const icon = ITEM_TYPE_ICONS[canonItemType(it['Тип'])] || '📄';
...
// Платные (Товар/Работа/Изделие) считаются в счёт; «Мат. заказчика» —
// отдельным блоком БЕЗ цен (как в документах, см. shared/vat.js).
// v4.48.0: тип «Изделие» (ранее «Товар+Работа») — изготовление под заказ:
// идёт в счёт и НАКЛАДНУЮ, в АКТ не попадает (акт собирает только «Работа»).
// Легаси-значение «Товар+Работа» в данных приводится к «Изделие» (canonItemType).
const ITEM_TYPES_PAID = ['Товар', 'Работа', 'Изделие'];
const ITEM_UNITS = ['шт.', 'л.', 'кг.', 'г.', 'к-т'];
const ITEM_TYPE_ICONS = { 'Товар': '📦', 'Работа': '🔧', 'Изделие': '🏭', 'Мат. заказчика': '🧱' };
const canonItemType = (t) => (t === 'Товар+Работа' ? 'Изделие' : t);
