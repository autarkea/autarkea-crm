...
function emptySession() {
    return {
        state: 'idle',
        // v4.53.0: «откуда пришёл» при открытии карточки задачи. Рендеры списков
        // задач кладут сюда { kind: 'tl'|'td'|'hl'|'pt', page, projectId? }, чтобы
        // view_back/done_ возвращали в ТОТ ЖЕ список («На сегодня», «История»,
        // «Задачи проекта»), а не в «Все задачи». null = контекста нет (фоллбэк).
        taskListCtx: null,
        // v4.54.0: контекст справочника (Контакты/Юрлица) для возврата из карточки.
        // Рендеры списков кладут null, рендеры результатов поиска — { query }.
        // «⬅️ Назад» из карточки (ccard_back/lcard_back) возвращает в результаты
        // поиска, а не на страницу полного списка (иначе найденное теряется).
        contactBookCtx: null,
        legalBookCtx: null,
        taskDraft: {
            title: '',
        ...
        itemDraft: {
            ...
            type: null,       // Товар/Работа/Изделие/Мат. заказчика
            name: '',
            unit: 'шт.',
