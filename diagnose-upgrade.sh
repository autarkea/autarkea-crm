#!/bin/bash
# diagnose-upgrade.sh v1.2.1 — Полная диагностика после обновления NocoDB
# v1.2.1: фикс ложного ❌ «Zombie Chrome процессов: 0» — `grep -c || echo 0`
#   давал ZOMBIE="0\n0" при нуле зомби (grep -c печатает «0» и exit 1).
# v1.2.0: версия больше НЕ захардкожена («была 2026.06.2» протухла после
#   2026.08.0): ожидаемая версия читается из docker-compose.yml (источник правды
#   пина образа), переопределяется EXPECTED_NOCO_VERSION. При рассинхроне
#   контейнер/compose — fail. Это делает скрипт пригодным для ЛЮБОГО апгрейда.
...
echo -e "${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  🔍 ПОЛНАЯ ДИАГНОСТИКА ПОСЛЕ ОБНОВЛЕНИЯ NocoDB v1.2.1     ║${NC}"
...
if [ "$NOCO_VERSION" != "недоступен" ]; then
    ...
    info "Актуальная версия (рекомендация NocoDB): $RELEASE"

    # Ожидаемая версия — из docker-compose.yml (пин образа, источник правды).
    # Переопределяется переменной EXPECTED_NOCO_VERSION.
    EXPECTED="${EXPECTED_NOCO_VERSION:-}"
    if [ -z "$EXPECTED" ]; then
        COMPOSE_FILE="${INSTALL_DIR:-$(pwd)}/docker-compose.yml"
        EXPECTED=$(grep -m1 'image: nocodb/nocodb:' "$COMPOSE_FILE" 2>/dev/null | sed -n 's/.*nocodb\/nocodb:\([^ "]*\).*/\1/p')
    fi
    if [ -z "$EXPECTED" ]; then
        warn "Не удалось определить ожидаемую версию из docker-compose.yml"
    elif [ "$CURRENT" = "$EXPECTED" ]; then
        pass "Версия контейнера совпадает с docker-compose.yml: $CURRENT"
    else
        fail "Рассинхрон: контейнер = $CURRENT, docker-compose.yml = $EXPECTED"
    fi
else
...
if docker ps --format '{{.Names}}' | grep -q "bot"; then
    # v1.2.1: `grep -c` при 0 совпадений печатает «0» и возвращает exit 1, а `|| echo 0`
    # дописывал вторую строку → ZOMBIE="0\n0" и тест падал (ложный ❌ при 0 зомби).
    ZOMBIE=$(docker exec printed4u-bot ps aux 2>/dev/null | grep -c "chrome.*defunct" || true)
    if [ "${ZOMBIE:-0}" -eq 0 ]; then
...
if [ "$FAIL" -eq 0 ]; then
    ...
    echo -e "${RED}║   Разбери каждый FAIL и при необходимости откати         ║${NC}"
    echo -e "${RED}╚═══════════════════════════════════════════════════════════╝${NC}"
fi
