#!/bin/bash
# ============================================================================
# upgrade.sh v1.3.0 — движок обновления установленной Printed4U CRM
...
if [ "$DRY_RUN" = false ] && [ "$SKIP_MODULES" = false ]; then
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # v4.61.0 (Проблема 129): ДОКАТКА МОДУЛЯ SAMBA.
    # install.sh ставит Samba при УСТАНОВКЕ, но upgrade.sh его не перезапускал —
    # клиенты, жившие на апгрейде, навсегда оставались на старой модели
    # (force user = владелец данных, без пользователя smbuser). Клиентский кейс
    # 12.09.2026: Samba v1.x, защиты «SMB ≠ владелец» (v2.0.0) не было вовсе —
    # из Проводника можно было переименовать/удалить папку проекта.
    # Ветки: smbuser есть → всё актуально (пропуск); нет → докатываем модуль.
    # ═══════════════════════════════════════════════════════════════════════
    if command -v smbd >/dev/null 2>&1 && [ -f /etc/samba/smb.conf ]; then
        if id smbuser >/dev/null 2>&1; then
            log "${GREEN}   📂 Samba: модель актуальна (пользователь smbuser есть) — пропуск${NC}"
        elif [ -f "$INSTALL_DIR/modules/samba-install.sh" ]; then
            log "${YELLOW}   ⚠️  Samba УСТАРЕЛА: нет пользователя smbuser (старая модель 'force user = владелец').${NC}"
            log "${YELLOW}      Докатываю modules/samba-install.sh. ВНИМАНИЕ: логин и пароль SMB ИЗМЕНЯТСЯ,${NC}"
            log "${YELLOW}      а сетевой диск в Windows нужно будет переподключить (net use * /delete /y).${NC}"
            # SAMBA_UPGRADE=1 — неинтерактивно (без паузы «запиши пароль»): иначе апгрейд зависает.
            if SAMBA_UPGRADE=1 bash "$INSTALL_DIR/modules/samba-install.sh" 2>&1 | sed 's/^/   /'; then
                log "${GREEN}   ✅ Samba докатана до актуальной модели (smbuser + защита ФС).${NC}"
                log "${YELLOW}      🔑 Логин/пароль — в выводе выше. Не успел записать? Смени: sudo smbpasswd smbuser${NC}"
            else
                log "${RED}   ❌ samba-install.sh завершился с ошибкой — Samba НЕ докатана.${NC}"
                log "${RED}      Проверь вывод выше и бэкап /etc/samba/smb.conf.backup.*; остальная установка продолжена.${NC}"
            fi
        fi
    fi
else
