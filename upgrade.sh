#!/bin/bash
# ============================================================================
# upgrade.sh v1.4.0 — движок обновления установленной Printed4U CRM
...
# ============================================================================
# Изменения v1.4.0 (v4.63.0):
# - ДОКАТКА МОДУЛЕЙ ОБОБЩЕНА: вместо захардкоженного списка (health-alert,
#   fix-fs + отдельная ветка Samba) — единый modules/ensure-modules.sh.
#   Он сверяет sha256 каждого «установленного» модуля с последним применённым:
#   изменился → AUTO-модули докатываются сами, интерактивные — предупреждаются
#   с командой. Так ни один фикс модуля больше не «застревает» у клиентов.
# ============================================================================
# Изменения v1.3.0 (v4.61.0):
# - ДОКАТКА МОДУЛЯ SAMBA. install.sh ставил Samba один раз при УСТАНОВКЕ, а
#   upgrade.sh его не перезапускал → клиенты на апгрейде навсегда оставались
#   на старой модели (force user = владелец данных, без smbuser). Это не только
#   лишало защиты v2.0.0, но и было РИСКОМ: SMB-клиент = владелец данных, то есть
#   папку проекта можно было переименовать/удалить из Проводника. Теперь, если
#   Samba установлена, а smbuser нет — модуль докатывается (идемпотентно),
#   с громким предупреждением: логин/пароль SMB изменятся.
# ============================================================================
...
# ═══════════════════════════════════════════════════════════════════════════
log "${BLUE}═══════════════════════════════════════════════════════════${NC}"
# Версия самого движка — читаем из строки-заголовка (шапка файла: `# upgrade.sh vX.Y.Z`),
# чтобы баннер и шапка не расходились (было: шапка v1.4.0, а баннер печатал v1.2.0).
UPGRADE_VER=$(sed -n 's/^# upgrade\.sh \(v[0-9][0-9.]*\).*/\1/p' "${BASH_SOURCE[0]}" | head -1)
UPGRADE_VER="${UPGRADE_VER:-?}"
log "${BLUE}🚀 upgrade.sh ${UPGRADE_VER} — обновление установленной CRM${NC}"
...
# ═══════════════════════════════════════════════════════════════════════════
# ДОКАТКА МОДУЛЕЙ (v4.63.0, Проблема 131 — обобщение Проблем 117/129).
# install.sh ставит модули ОДИН раз при УСТАНОВКЕ, а upgrade.sh раньше
# перезапускал лишь два из них (health-alert, fix-fs) — поэтому любой фикс
# остальных модулей «застревал»: боевой кейс 12.09.2026, Samba у клиента
# навсегда осталась v1.x (без smbuser и без защиты ФС).
# Теперь единая точка: modules/ensure-modules.sh сверяет sha256 каждого
# «установленного» модуля с последним применённым (состояние —
# /mnt/data/nocodb-data/module-state.txt):
#   - изменился и модуль неинтерактивный (AUTO: health-alert, fix-fs, samba,
#     firewall) → докатывается автоматически;
#   - изменился, но модуль интерактивный (bot/email/backup/tailscale/HTTPS/
#     Cloudflare/heartbeat/formulas) → печатается предупреждение с командой.
# Сбой докатки НЕ прерывает обновление (продолжаем, сообщаем в лог).
# ═══════════════════════════════════════════════════════════════════════════
if [ "$DRY_RUN" = false ] && [ "$SKIP_MODULES" = false ]; then
    if [ -f "$INSTALL_DIR/modules/ensure-modules.sh" ]; then
        if ! bash "$INSTALL_DIR/modules/ensure-modules.sh" 2>&1 | sed 's/^/   /'; then
            log "${YELLOW}   ⚠️  Часть модулей не докаталась — см. вывод выше (обновление продолжается)${NC}"
        fi
    else
        # Страховка для старой укладки: докатываем хотя бы host-cron (Проблема 117)
    ...
    log "${YELLOW}   📡 Докатка модулей пропущена (--skip-modules / --db-only / --dry-run)${NC}"
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    # Пост-проверка СОСТАВА (v4.70.0). Маркер версии говорит «до какой дельты догнали»,
    # но не «какой получился состав»: дельта может идемпотентно пропустить шаг (поле с
    # тем же именем уже есть, но другого смысла) — и маркер при этом честно поднимется.
    # Поэтому сверяем состав с эталоном. Это ПРЕДУПРЕЖДЕНИЕ (не err): часть расхождений
    # лечится не дельтами, а ручной правкой UI, и валить апдейт из-за них нельзя.
    if [ -f "$INSTALL_DIR/modules/schema-fingerprint.sh" ]; then
        log "${BLUE}🧬 Пост-проверка состава схемы (слепок против эталона)...${NC}"
        SET_RC=0
        SET_OUT=$(NOCO_DB="$NOCO_DB" TEMPLATE="$TEMPLATE" INSTALL_DIR="$INSTALL_DIR" \
            bash "$INSTALL_DIR/modules/schema-fingerprint.sh" check 2>&1) || SET_RC=1
        echo "$SET_OUT" | sed 's/^/   /'
        if [ "$SET_RC" -ne 0 ]; then
            log "${YELLOW}   ⚠️  Состав схемы отличается от эталона (см. список выше).${NC}"
            log "${YELLOW}      Маркер версии при этом синхронен — расхождение НЕ про версию, а про состав:${NC}"
            log "${YELLOW}      обычно поле с тем же именем, но другим смыслом (список vs одиночная связь),${NC}"
            log "${YELLOW}      либо правка, сделанная только в UI NocoDB и не оформленная дельтой.${NC}"
            log "${YELLOW}      Догон: дельта U<NNN> + export-template.sh (upgrades/README.md).${NC}"
        fi
    fi
fi
