...
# ============================================================================
# 🚫 GUARD: НЕ запускать установщик под sudo/root (v4.62.0, Проблема 130)
# `sudo bash install.sh` тихо ломает ВЛАДЕЛЬЦА ДАННЫХ: $USER → root, `id -u` → 0,
# поэтому в .env уезжает APP_UID=0, а /mnt/data достаётся root:root. Тогда контейнеры
# (вебхук) создают папки от root, а SMB-пользователь smbuser (он в группе обычного
# пользователя) не может писать НИКУДА — симптом «создал папку, а файлы не закинуть».
# Скрипт сам вызывает sudo там, где нужно, — запускать его надо ОТ ОБЫЧНОГО ПОЛЬЗОВАТЕЛЯ.
#
# Исключение — сервер, где единственная учётка = root (типичный VPS): там альтернативы
# нет, поэтому после ЯВНОГО подтверждения установка разрешается (но риск остаётся:
# владельцем данных будет root).
# ============================================================================
if [ "$(id -u)" -eq 0 ]; then
    if [ -n "${SUDO_USER:-}" ] && [ "${SUDO_USER}" != "root" ]; then
        echo -e "${RED}❌ Установщик запущен через sudo. Так нельзя.${NC}"
        echo -e "${YELLOW}   Запуск от root ломает владельца /mnt/data и APP_UID/APP_GID в .env:${NC}"
        echo -e "${YELLOW}   контейнеры начнут писать от root, а SMB-пользователь не сможет писать файлы.${NC}"
        echo -e "${CYAN}   Запусти без sudo, от своего пользователя:${NC}"
        echo -e "${CYAN}      exit        # выйти из sudo-сессии, если ты в ней${NC}"
        echo -e "${CYAN}      bash install.sh${NC}"
        exit 1
    fi
    echo -e "${YELLOW}⚠️  Ты root. Скрипт сам вызывает sudo для нужных шагов.${NC}"
    echo -e "${YELLOW}   Установка от root сделает владельцем данных root — это РИСК для Samba${NC}"
    echo -e "${YELLOW}   и прав файлов. Если можешь — создай пользователя и запусти от него.${NC}"
    read -p "   Продолжить установку от root? (y/N): " allow_root
    if [[ "$allow_root" != "y" && "$allow_root" != "Y" ]]; then
        echo -e "${CYAN}   Отменено. Рекомендуется: sudo adduser crm && sudo usermod -aG sudo crm, затем вход под crm.${NC}"
        exit 1
    fi
fi
...
#
# 🆕 v4.62.0 (Проблема 130): SETGID (бит 2xxx) на папках-каркасах.
# Setgid заставляет НОВЫЕ элементы наследовать ГРУППУ РОДИТЕЛЯ, а не GID процесса.
# Без него папку, созданную контейнером с другим GID, SMB-пользователь не сможет
# писать (класс проблем «создал папку — файлы не закинуть»). См. docs → «Защита ФС».
sudo chmod 2755 $DATA_DIR            # /mnt/data: новые элементы наследуют группу данных
sudo chmod 2755 $DATA_DIR/projects
sudo chmod 2755 $DATA_DIR/clients
sudo chmod 2755 $DATA_DIR/noco-static
sudo chmod 2775 $DATA_DIR/noco-static/pdfs  # PDF пишут контейнеры (группа + setgid)
sudo chmod 0700 $DATA_DIR/backups
sudo chmod 2755 $DATA_DIR/logs  # 📮 v4.49.0: outbox-очередь бота (пишет контейнер от ${APP_UID})
sudo chmod 2755 $DATA_DIR/nocodb-data
...
# ============================================================================
# 📦 v4.63.0 (Проблема 131): КАЛИБРОВКА СОСТОЯНИЯ МОДУЛЕЙ.
# Что уже применено — фиксируем по хешу файлов. Дальше upgrade.sh докатит
# ТОЛЬКО изменившиеся модули (и не будет «ложных» предупреждений про остальные).
# ============================================================================
if [ -f "modules/ensure-modules.sh" ]; then
    bash modules/ensure-modules.sh --calibrate 2>&1 | sed 's/^/   /'
else
    echo -e "${YELLOW}⚠️  modules/ensure-modules.sh не найден — состояние модулей не откалибровано${NC}"
fi
